---
layout: post
title: Grafana v13.0.9
date: '2026-09-17'
category: k8s-releases
tool: Grafana
version: v13.0.9
prerelease: false
url: https://github.com/grafana/grafana/releases/tag/v13.0.9
tags:
- kubernetes
- cncf
- grafana
---

## Grafana v13.0.9

**Version**: v13.0.9
**Type**: Stable
**Release**: [https://github.com/grafana/grafana/releases/tag/v13.0.9](https://github.com/grafana/grafana/releases/tag/v13.0.9)

### Changelog

[Download page](https://grafana.com/grafana/download/13.0.9)
[What's new highlights](https://grafana.com/docs/grafana/latest/whatsnew/)

### Security

- Security: Fix CVE-2026-15815
- Security: Fix CVE-2026-76154
- Security: Fix CVE-2026-79656

### Bug fixes

- **Dashboards:** Preserve query variable refresh setting on v2 dashboard import [#132088](https://github.com/grafana/grafana/pull/132088), [@grafana-writer[bot]](https://github.com/grafana-writer[bot])
